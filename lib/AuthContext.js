...
export const AuthProvider = ({children}) => {
  ...
  // Check user data on app load – run once on mount only
  useEffect(() => {
    ...
    checkUser();
  }, []); // Empty array: only run once on mount, userData is fetched and set inside the effect
  ...
  const logout = async () => {
    try {
      ...
      // Clear persisted Redux state so stale data is not shown on next login
