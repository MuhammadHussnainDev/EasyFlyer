...
export const GiftCardsComponent = () => {
  ...
  useEffect(() => {
    const loadGiftCards = async () => {
      try {
        const allCoupons = await fetchAllCouponGifts();
        // Filter out expired gift cards using endDate
        const validCoupons = filterExpiredContent(allCoupons, 'endDate');
        console.log('Fetched Gift Cards:', allCoupons.length, 'Valid Gift Cards:', validCoupons.length);
        setGiftCards(validCoupons);
      } catch (error) {
